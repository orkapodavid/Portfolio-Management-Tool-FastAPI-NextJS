...
import { riskGetRiskInputs } from "@/app/clientService";
...
export default function RiskInputsPage() {
  ...
  const [rows, setRows] = useState<RiskInputRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const response = await riskGetRiskInputs({
    ...
    if (error) {
      ...
      setErrorMessage("Failed to load risk inputs.");
      setIsLoading(false);
      return;
    }
    setRows((getApiData(response) as RiskInputRow[]) ?? []);
