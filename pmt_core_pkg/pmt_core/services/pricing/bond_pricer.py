...
class BondPricer:
    ...
    def price_bond(
        ...
        time_premium = (
            notional * volatility * math.sqrt(max(time_to_maturity_years, 0.001)) * 0.05
        )
    ...
    def generate_yields(self) -> np.ndarray:
        ...
        return np.linspace(self.yield_range[0], self.yield_range[1], self.num_points)
    ...
    def generate_curve_data(self, x_axis: str, y_axis: str) -> dict[str, np.ndarray]:
        """
    ...
    def generate_surface_data(self, z_axis: str) -> dict[str, np.ndarray | str]:
        """
